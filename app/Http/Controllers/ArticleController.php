...
class ArticleController extends Controller
{
	...
	public function index_publish(): Response
	{
		$data = [
			'title' => 'berita rilis',
			'list'	=> route('article.list', ['publish'=>'publish', 'category'=>request()->category ?? null]),
            'create'=> ['action' => route('article.create')],
			'delete'=> ['action' => route('article.destroy', 0), 'message' => 'Hapus berita?']
		];

		return response()->view('panel.article.index', compact('data'));
	}
	...
	public function index_draft(): Response
	{
		$data = [
			...
			'list'	=> route('article.list', ['publish'=>'draft', 'category'=>request()->category ?? null]),
            'create'=> ['action' => route('article.create')],
			'delete'=> ['action' => route('article.destroy', 0), 'message' => 'Hapus berita dari draft?']
		];
	...
	public function index_schedule(): Response
	{
		$data = [
			...
			'list'	=> route('article.list', ['publish'=>'schedule', 'category'=>request()->category ?? null]),
            'create'=> ['action' => route('article.create')],
			'delete'=> ['action' => route('article.destroy', 0), 'message' => 'Hapus berita terjadwal?']
		];
	...
	public function list(Request $request, string $publish, string $category = null): JsonResponse
	{
		...
		$totalDataRecord = Article::wherePublish($publish);
		...
		if (empty($request->input('search.value'))) :
			$datatable = Article::wherePublish($publish)->withCount('comment');
			...
			$datatable =  Article::wherePublish($publish)->where('title', 'LIKE', "%{$search_text}%")->withCount('comment');
			$datatable = ($filter===true) ? $datatable->where('article_category_id', $article_category->id) : $datatable;
			$datatable = $datatable->offset($start_val)->limit($limit_val)->get();
			$totalFilteredRecord = Article::wherePublish($publish)->where('title', 'LIKE', "%{$search_text}%");
		...
		if (!empty($datatable)) :
			foreach ($datatable as $key => $item) :
				...
				$data_val[$key]['title'] = anchor(text:$item->title, href:route('article.edit', $item->id), class:['d-inline-block', 'fw-bold'])."<br/><span class=\"badge me-1\" style=\"font-size:10px;background-color:{$item->article_category->color}\">{$item->article_category->title}</span><span style=\"font-size:12px;\">Editor: <b>{$item->user->name}</b></span> | ".anchor(href:route('article.ad', $item->id), text:"<i class=\"bx bx-objects-vertical-bottom\"></i> Atur iklan", class:['text-primary', 'small', 'text-nowrap'])." | ".anchor(href:route('article.comment', $item->id), text:"<i class=\"bx bx-comment\"></i> Lihat komentar ({$item->comment_count})", class:['text-primary', 'small', 'text-nowrap']);
	...
	public function create(): Response
	{
		...
		$data = [
			'title'	=> 'buat berita',
			'form'	=> ['action' => route('article.store'), 'class' => 'form-insert']
		];

		return response()->view('panel.article.form', compact('data', 'article', 'article_related', 'article_category'));
	}
	...
	public function store(Request $request): JsonResponse
	{
		...
		return response()->json([
			...
			'redirect'	=> ['type' => 'assign', 'value' => route('article.'.$request->publish)]
		]);
	}
	...
	public function edit(string $id): Response
	{
		...
		$data = [
			'title'	=> 'edit berita',
			'form' => ['action' => route('article.update', $id), 'class' => 'form-update']
		];

		return response()->view('panel.article.form', compact('data', 'article_category', 'article_related', 'article'));
	}
