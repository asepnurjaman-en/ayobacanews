	...
	public function dashboard(Request $request): Response
	{
		...
		$dashboard = [
			[
				...
				'data'	=> Article::where('publish', 'publish')->count()
			],
			[
				'icon'	=> 'bx bx-news',
				'title'	=> 'berita draft',
				'url'	=> route('article.draft'),
				'data'	=> Article::where('publish', 'draft')->count()
			],
			[
				'icon'	=> 'bx bx-news',
				'title'	=> 'berita terjadwal',
				'url'	=> route('article.schedule'),
				'data'	=> Article::where('publish', 'schedule')->count()
			],
		];
		$comment_waiting = ArticleComment::wherePublish('draft')->count();
		$shortcut = [
			[
				'card'	=> 'danger',
			...
			[
				...
				'data'	=> User::whereRole('editor')->count()
			],
			[
				'card'	=> 'warning',
				...
				'data'	=> ArticleCategory::count()
			],
		];
		$data = ['title' => 'Dasbor'];

		return response()->view('panel.index', compact('data', 'greating', 'dashboard', 'shortcut', 'comment_waiting'));
	}
	...
	public function dashboard_ae()
	{
		...
		$dashboard = [
			[
				...
				'data'	=> Article::where('publish', 'publish')->where('user_id', Auth::user()->id)->count()
			],
			[
				'icon'	=> 'bx bx-news',
				'title'	=> 'draft saya',
				'url'	=> route('ae.article.draft'),
				'data'	=> Article::where('publish', 'draft')->where('user_id', Auth::user()->id)->count()
			],
			[
				'icon'	=> 'bx bx-news',
				'title'	=> 'berita terjadwal',
				'url'	=> route('ae.article.schedule'),
				'data'	=> Article::where('publish', 'schedule')->where('user_id', Auth::user()->id)->count()
			],
		];
		$data = ['title' => 'Dasbor'];

		return response()->view('editor.index', compact('data', 'greating', 'dashboard'));
	}
