...
class ArticleCategoryController extends Controller
{
    ...
    public function index(): Response
    {
        $data = [
            'title' => 'label artikel',
            'list'  => route('articlecategory.list'),
            'create'=> ['action' => route('articlecategory.create')],
            'delete'=> ['action' => route('articlecategory.destroy', 0), 'message' => 'Hapus kategori artikel?']
        ];

        return response()->view('editor.article.index-category', compact('data'));
    }
    ...
    public function list(Request $request): JsonResponse
    {
		...
		if (!empty($datatable)) :
			foreach ($datatable as $key => $item) :
				...
				$data_val[$key]['title'] = anchor(text:$item->title, href:route('articlecategory.edit', $item->id))."<br/><span class=\"badge me-1\" style=\"font-size:10px;background-color:{$item->color}\">{$item->color}</span>";
    ...
    public function create(): Response
    {
        $data = [
            ...
            'form'  => ['action' => route('articlecategory.store'), 'class' => 'form-insert']
        ];

        return response()->view('editor.article.form-category', compact('data'));
    }
    ...
    public function store(Request $request): JsonResponse
    {
		...
		return response()->json([
            ...
            'redirect'  => ['type' => 'assign', 'value' => route('articlecategory.index')]
        ]);
    }
    ...
    public function edit(string $id): Response
    {
        ...
        $data = [
            ...
            'form'  => ['action' => route('articlecategory.update', $id), 'class' => 'form-update']
        ];

        return response()->view('editor.article.form-category', compact('data', 'article_category'));
    }
