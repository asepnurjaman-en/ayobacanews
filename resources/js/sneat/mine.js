...
if ($(".select2-news").length > 0) {
	function formatStateNews (state) {
		if (!state.id) { return state.text; }
		var $state = $('<span><i class="bx bx-plus-circle"></i> ' + state.text + '</span>');
	   	return $state;
	};
	let action = $(".select2-news").data('url');

	$(".select2-news").select2({
		placeholder: "Pilih berita",
		templateResult: formatStateNews,
		ajax: {
			url : action,
			dataType: 'json',
